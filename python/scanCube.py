...
# > python3.8 scanCube.py
# > python3.8 scanCube.py --dev=help
# > python3.8 scanCube.py --model=models/model1.png
# > python3.8 scanCube.py --model=models/model1.png --dev=dir:imagenes/test*.png 
...
def drawGrid(cube, moves):
    ...
    if moves != []: 
        ...
        drawTextS(grid_frame, 'FACES: ' + str(len(cube)) + '/6',
                 (GRID_MARGIN, G_CUBE_S*2+GRID_SIZE*2), 0.75, GRID_TEXT_COL, 2)
    # Text of return key and indication
    drawTextS(grid_frame, 'Press [ENTER] when finished',
                 (G_CUBE_S+GRID_SIZE, GRID_MARGIN*2), 0.75, GRID_TEXT_COL, 2)
    drawTextS(grid_frame, '(To display the same colors on the visualizer,',
                (G_CUBE_S+GRID_SIZE, GRID_SIZE+GRID_MARGIN), 0.52, [30, 30, 30], 1)
    drawTextS(grid_frame, 'start with yellow face front and red face down)',
                (G_CUBE_S+GRID_SIZE, GRID_SIZE*2), 0.52, [30, 30, 30], 1)
...
#                           MODEL IMAGE
###############################################################################
# Model default image file
model_file = 'models/model1.png'
# If model file is passed as an argument
opts, args = getopt.getopt(sys.argv[1:],"dm:",["dev=", "model="])
for opt, arg in opts:
    if opt in ("-m", "--model"):
        model_file = arg
...
for key, frame in autoStream():
    ...
    if key == ord('f'):         # Remove last face scanned
        ...
        if not SHOW_GRID: cv.destroyWindow('color grid')
            ...
            cv.namedWindow('color grid') 
            cv.moveWindow('color grid', 700, 100)
    elif key == 13:     # 13 is RETURN key code
        if moves:
            moves = str(moves)
